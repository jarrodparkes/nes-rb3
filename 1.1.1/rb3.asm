...
; v1.1.1	- Changed mapper to NROM-128, fixed graphics bugs, consolidated graphics
;------------------------------------------------------------------------------------------/
...
; special variables
SPRITE_RAM		EQU $0200	; entire page for sprite data
; [END VARIABLES]
;------------------------------------------------------------------------------------------/
...
;Moved from below JSR to here.
	LDA #$80
	STA PPU_CTRL
;

	;CHANGED FROM WAITVBLANK
	JSR WaitTillNMI			; wait for vblank
...
LoadParkesBack:	
	...
	JSR LoadPalette
	JSR SetFadeIn
	LDA #%10001000 ;Set up graphics RAM for writing via WaitFadeDone
	STA <PPU_CTRL_RAM
	JSR WaitTillNMI			; we've finished drawing graphics, vblank has hit, turn screen on

	LDA #%00011110   		; enable background, no clipping on left side, deleted a updatepalette and waitnmi here and somewhere else.
...
AtParkes:
	...
	CMP #FADE_MODE_OUT
	BEQ ExitParkes
;NOTE: CHANGD FROM JMP TO BNE, SAVES 1 BYTE.
	BNE AtParkes

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	
ExitParkes:
	JSR WaitTillNMI ;THIS GOES FIRST. FadeDone was happening during rendering whacking out the OAM/other stuff.
	JSR WaitFadeDone
	
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	LDA #0 ;MOVED HERE FROM BELOW.
	STA PPU_MASK

	LDA #$FF
	TAX ;X=FF to clear sprites. Just getting rid of the sprites from the gameplay screen.
.ClearSpriteRAM:
	STA SPRITE_RAM,X
	DEX
	BNE .ClearSpriteRAM
...
;ResetPPUAdr
	JSR SetFadeIn
	LDA #%10011000 ;Set up graphics RAM for writing via WaitFadeDone
	STA <PPU_CTRL_RAM
	JSR WaitTillNMI			; we've finished drawing graphics, vblank has hit, turn screen on
	
;Changed to enable sprites too.
...
ExitTitle:
	JSR WaitTillNMI ;FIRST
	JSR WaitFadeDone ;Then wait and do pallete stuffs.
...
WaitFadeDone:
	JSR UpdatePalette
	JSR UpdateOAM
	JSR ClearScroll ;Also sets PPUCtrl.
	JSR WaitTillNMI
	LDA <FADE_MODE
	CMP #FADE_MODE_NONE
	BNE WaitFadeDone
...
; [SECOND 8KB PGR-ROM]
	...
	.org $E000					; start location of bank 1 ($E000 in CPU memory space)
		...
		;vert	;tile	;attr	;horz
MetaBlockCollision:
	.db $88,	$8E,	$03,	$88		; top-left corner
	.db $88,	$8E,	$03,	$97		; top-right corner
	.db $97,	$8E,	$03,	$88		; bottom-left corner
	.db $97,	$8E,	$03,	$97		; bottom-right corner

MetaBlock:
	.db $88,	$5C,	$01,	$88		; top-left
	.db $88,	$5D,	$01,	$90		; top-right
	.db $90,	$5E,	$01,	$88		; bottom-left
	.db $90,	$5F,	$01,	$90		; bottom-right

MetaPickUp0:
	.db $53,	$5C,	$00,	$53		; top-left
	.db $53,	$5D,	$00,	$5b		; top-right
	.db $5b,	$5E,	$00,	$53		; bottom-left
	.db $5b,	$5F,	$00,	$5b		; bottom-right

MetaTimer:
	.db $cf,	$00,	$00,	$48		; hundreds digit
	.db $cf,	$00,	$00,	$50		; tens digit
	.db $cf,	$00,	$00,	$58		; ones digit
...
; [SYSTEM INTERRUPTS]	
	...
	.dw RTI     			; external interrupt IRQ is not used, assign null
; [END INTERRUPTS]
;------------------------------------------------------------------------------------------/
